...
namespace Netherlands3D.LayerSystem
{
	public class TileHandler : MonoBehaviour
	{
		...
		void Update()
		{
			...
			CameraExtents.CalculateExtent();
			viewRange = GetViewRange();
			cameraPosition = GetCameraPosition();
			var cameraPositionRD = CoordConvert.UnitytoRD(Config.activeCamera.transform.position);
		...
		{
			Extent extent = CameraExtents.cameraExtent;
		...
		private void GetTileDistancesInView(List<int> tileSizes, Vector4 viewRange, Vector3Int cameraPosition)
		{
			...
			//{
				GeometryUtility.CalculateFrustumPlanes(Config.activeCamera, cameraFrustumPlanes);
			//}
			tileDistances.Clear();
